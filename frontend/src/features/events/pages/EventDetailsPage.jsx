...
import { Button, Card, InfoMessage } from "../../../shared/components";
...
export default function EventDetailsPage() {
  ...
  async function load() {
    ...
    try {
      ...
      if (data?.impact_analysis?.analysis) {
        setAnalysis(data.impact_analysis.analysis);
      }
    } catch (e) {
  ...
  async function runAnalysis() {
    ...
    try {
      ...
      if (!res?.success) {
        throw new Error(
          res?.message || t("eventDetailsPage.errorRequestFailed"),
        );
      }
      ...
      if (res?.analysis) {
        setImpact((prev) => ({
          ...
          is_analyzed: true,
          overall_impact:
            res.analysis.overall_impact || prev?.overall_impact || "none",
          affected_products_count:
            Number(res.analysis.affected_products_count) ||
            prev?.affected_products_count ||
            0,
        }));
      }
    } catch (e) {
      ...
      if (detail?.error === "insufficient_event_data") {
        ...
        setError(
          extractApiMessage(e) || t("eventDetailsPage.errorAnalyzeFailed"),
        );
      }
    } finally {
      setAnalyzing(false);
  ...
  return (
    <div className="events-page">
      <div className="events-actions event-details-actions">
        ...
        {canEdit && (
          ...
          >
            {editing
              ? t("eventDetailsPage.btnCloseEdit")
              : t("eventDetailsPage.btnEditEvent")}
          </Button>
        )}
      </div>
        ...
        <Card>
          <div className="events-empty">
            <div className="events-empty-title">
              {t("eventDetailsPage.notFoundTitle")}
            </div>
            <div className="events-empty-subtitle">
              {t("eventDetailsPage.notFoundSubtitle")}
            </div>
          </div>
        </Card>
      ) : (
        <>
          {editing && canEdit && (
            ...
            >
              <EventEditForm
                ...
                onError={(msg) =>
                  setError(msg || t("eventDetailsPage.errorUpdateFailed"))
                }
              />
            </Card>
          )}
          ...
          >
            <div className="event-details-grid">
              ...
              <div className="detail-item">
                <div className="detail-label">
                  {t("eventDetailsPage.detailStatus")}
                </div>
              ...
              <div className="detail-item">
                <div className="detail-label">
                  {t("eventDetailsPage.detailDuration")}
                </div>
                <div className="detail-value">
                  {Number(event?.duration_days)
                    ? t("eventDetailsPage.detailDurationValue", {
              ...
              <div className="detail-item">
                <div className="detail-label">
                  {t("eventDetailsPage.detailRecurring")}
                </div>
                <div className="detail-value">
                  {event?.is_recurring
                    ? t("eventDetailsPage.detailRecurringYes", {
            ...
            {event?.description && (
              <div className="event-desc">
                <div className="detail-label">
                  {t("eventDetailsPage.detailDescription")}
                </div>
          ...
          >
            ...
            >
              <Button type="button" onClick={runAnalysis} disabled={analyzing}>
                {analyzing
                  ? t("eventDetailsPage.btnAnalyzing")
                  : t("eventDetailsPage.btnAnalyzeImpact")}
              </Button>
              ...
              <div style={{ marginBottom: 14 }}>
                <InfoMessage type="warning">
                  {analysisBlock.message ||
                    t("eventDetailsPage.analysisBlockFallback")}
                </InfoMessage>

                <div className="impact-grid" style={{ marginTop: 12 }}>
                  <div className="detail-item">
                    <div className="detail-label">
                      {t("eventDetailsPage.analysisBlockRequiredPeriod")}
                    </div>
                    <div className="detail-value">
                      {analysisBlock.required_event_period || "-"}
                    </div>
                  </div>

                  <div className="detail-item">
                    <div className="detail-label">
                      {t("eventDetailsPage.analysisBlockAvailableEnds")}
                    </div>
                    <div className="detail-value">
                      {analysisBlock.available_data_ends || "-"}
                    </div>
                  </div>

                  <div className="detail-item">
                    <div className="detail-label">
                      {t("eventDetailsPage.analysisBlockMissingPeriod")}
                    </div>
                    <div className="detail-value">
                      {analysisBlock.missing_period || "-"}
                    </div>
                  </div>

                  <div className="detail-item">
                    <div className="detail-label">
                      {t("eventDetailsPage.analysisBlockHint")}
                    </div>
                    <div className="detail-value">
                      {analysisBlock.hint || "-"}
                    </div>
