...
function getItemType(item) {
  const explicitType = String(item?.calendar_type || item?.source || item?.item_type || "").toLowerCase();
    ...
    <div className="cal">
      <div className="cal-legend" aria-label="Calendar legend">
        <div className="cal-legend-item">
          ...
          <span>{t("calendarPage.legendEvent", { defaultValue: "Event" })}</span>
        </div>

        <div className="cal-legend-item">
          <span className="cal-legend-dot cal-legend-dot-campaign" />
          <span>{t("calendarPage.legendCampaign", { defaultValue: "Campaign" })}</span>
        </div>
      </div>
              ...
              <div className="cal-items">
                {visibleItems.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      title={`${type === "campaign" ? "Campaign" : "Event"}: ${title}`}
                    >
                ...
                {hiddenCount > 0 ? (
                  <div className="cal-more">
                    +{hiddenCount} {t("calendarPage.more", { defaultValue: "more" })}
                  </div>
                ) : null}
              </div>
            </div>
